...
const URL_MATCHERS = {
  ...
  stylesheets: {
    stylesheetManager:
      "bigmachines.com/admin/ui/branding/edit_site_branding.jsp",
...
function matchesUrlPattern(url, patternKey, subPatternKey = null) {
  ...
  if (subPatternKey) {
    ...
    return typeof pattern === "string" ? url.includes(pattern) : false;
  }
}
...
chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    ...
    chrome.scripting.executeScript(
      ...
      () => {
        chrome.tabs.sendMessage(
          tabs[0].id,
          { action: "initializeDocumentHandlers" },
          (response) => {
            if (chrome.runtime.lastError) {
              logDebug(
                "Error initializing document handlers:",
  ...
  if (url) {
    ...
    if (
      matchesUrlPattern(url, topLevelFolder.stylesheets, "stylesheetManager")
    ) {
      bmSiteType = "stylesheets";
    ...
    if (
      ...
      logDebug(
        "Test buttons disabled due to matching admin commerce rules URL."
      );
    }
...
chrome.downloads.onDeterminingFilename.addListener(function (item, suggest) {
  logDebug(
    "Download detected, setting filename, subdomain, site type, and rule type:",
    item.filename,
    bmSiteSubDomain,
    bmSiteType,
    bmRuleType
    ...
    chrome.tabs.sendMessage(
      ...
      function (response) {
        if (chrome.runtime.lastError) {
          ...
          logDebug(
            "Received unload response, saving folder",
            response.foldername
          );
          logDebug("Received unload response, saving file:", response.filename);
          saveText(
            response.filename + "." + bmFileType,
            response.code,
            bmFileType
          );
        }
    ...
    chrome.tabs.sendMessage(
      ...
      function (response) {
        if (chrome.runtime.lastError) {
          ...
          logDebug(
            "Received unloadTest response, saving file:",
            response.filename
          );
          saveText(
            response.filename + ".test." + bmFileType,
            response.testCode,
            bmFileType
          );
        }
